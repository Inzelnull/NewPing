...
### 1. リアルタイム Ping 監視モニター (`Ping結果`)
登録された各監視対象の死活・応答時間をリアルタイムに追跡し、タイムライン形式で視覚化します。
![Ping監視モニター](./docs/images/ping_monitor.jpg)

### 2. 監視対象設定 (`Ping対象設定`)
`ping-list.config` の直接編集とリストプレビューに対応し、監視ホストを柔軟に管理可能です。
![Ping対象設定](./docs/images/ping_targets.jpg)

### 3. 詳細統計とCSVエクスポート (`Ping統計`)
全ターゲットの送信数、成功/失敗数、パケットロス率、最小/最大/平均RTTを自動集計し、ワンクリックでCSV出力可能です。
![Ping統計](./docs/images/ping_stats.jpg)

### 4. Traceroute（経路追跡）サブウィンドウ
目的のホストまでのネットワーク経路（各ホップルーターのIPとRTT）をサブウィンドウで調査可能です。
![Traceroute経路追跡](./docs/images/traceroute.jpg)
